...
//        List<Bill> bills = billRepository.findAll().collectList().block();
//        List<Basket> baskets = basketRepository.findAll().collectList().block();
...
//    @Test
//    void shouldGetById() {
//        BillView saved = billService.sale(modelMapper.map(basket, BasketView.class)).block();
//        BillView billView = billService.findById(user.getUserId(), saved.getBillId()).block();
...
//        billRepository.save(new Bill(null, customer, Collections.emptyList(), LocalDateTime.now(), 0L));
//        billRepository.save(new Bill(null, customer, Collections.emptyList(), LocalDateTime.now(), 0L));
//        List<BillView> billView = billService.findAll(user.getUserId()).collectList().block();
//        assertEquals(3, billView.size());
//    }
//
//    private void prepareData() {
//        user = userRepository.save(getUser()).block();
//        customer = customerRepository.save(getCustomer(null, user)).block();
